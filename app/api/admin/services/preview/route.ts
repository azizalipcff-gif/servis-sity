...
const MODERATION_AUDIT_ACTIONS = ["service.status_change"];

export async function GET(request: Request) {
  return withErrorCapture("admin.services.preview", async () => {
    ...
    const { data: service, error: svcError } = await supabase
      .from("services")
      .select(
        "*, categories!services_category_id_fkey(name_ar, name_fr, name_en), businesses(id, name, slug, owner_id, city)"
      )
      .eq("id", id)
      .maybeSingle();

    if (svcError || !service) return jsonError(404, "not_found");

    const ownerId = (service as { businesses?: { owner_id: string | null } | null }).businesses?.owner_id;
    ...
    return jsonOk({
      service,
      business: (service as { businesses?: unknown }).businesses ?? null,
