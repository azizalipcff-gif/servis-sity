...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: localizedLanguages(`/businesses/${canonicalCity}/${slug}`),
    },
    openGraph: {
      title: `${business.name} · Service City`,
      description,
      type: "website",
      url,
    ...
    <div className="pb-24 lg:pb-8">
      ...
      >
        ...
        {business.categories ? (
          ...
          <span className="max-w-40 truncate">{t("home")}</span>
        )}
        {business.city && (
          <>
            <ChevronRight
              ...
              aria-hidden
            />
            <span className="max-w-32 truncate">{business.city}</span>
          </>
        )}
        <ChevronRight
          className="size-3.5 shrink-0 text-muted-foreground/60 rtl:rotate-180"
        ...
        />
        <Link
          href={`/businesses/${canonicalCity}/${business.slug}`}
      ...
      <div className="container-site mt-8 grid items-start gap-8 py-8 lg:grid-cols-3 lg:gap-8">
        <div className="min-w-0 space-y-12 lg:col-span-2 lg:space-y-14">
          <div id="about" className={sectionCls}>
            <FadeIn>
              <BusinessInfo business={business} locale={locale as Locale} />
            </FadeIn>
          </div>

          <div id="services" className={sectionCls}>
            <FadeIn>
          ...
          {products.length > 0 && (
            <div id="products" className={sectionCls}>
              <FadeIn>
                <BusinessProducts products={products} business={business} />
              </FadeIn>
            </div>
          )}
          ...
          <div id="reviews" className={sectionCls}>
            <FadeIn>
        ...
        <aside className="space-y-6 lg:col-span-1 lg:sticky lg:top-28 lg:self-start">
...
function schema(business: BusinessDetail, locale: string) {
  const base = `${siteUrl()}${businessPath(locale, business)}`;
  const offers =
