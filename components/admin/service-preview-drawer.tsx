...
export function ServicePreviewDrawer({
  ...
  const [data, setData] = useState<ServicePreview | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/admin/services/preview?id=${encodeURIComponent(serviceId)}`);
      if (res.ok) {
        const json = await res.json();
        setData(json as ServicePreview);
      } else {
        setData(null);
      }
    } catch {
  ...
  async function moderate(status: string, statusNote?: string | null) {
    ...
    try {
      ...
      setData((prev) =>
        prev ? { ...prev, service: { ...prev.service, status, status_note: statusNote ?? null } } : prev,
      );
      setRejectOpen(false);
      setNote("");
      onModerated(serviceId, status);
      void load();
    } finally {
      setBusy(false);
    }
  ...
  const gallery = data?.service.gallery ?? [];
  const images = [data?.service.photo_url, ...gallery].filter(
  ...
  const priceFmt = (v: number) =>
    new Intl.NumberFormat(locale === "ar" ? "ar-MA" : locale, {
      style: "currency",
      currency: data?.service.currency ?? "MAD",
    }).format(v);
        ...
        <header className="flex items-center justify-between gap-3 border-b px-5 py-4">
          <div className="flex min-w-0 items-center gap-3">
            <div className="min-w-0">
              <h3 className="truncate font-semibold">{data?.service.name ?? fallbackName}</h3>
              {data && <StatusBadge status={data.service.status} t={t} />}
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          ...
          {loading || !data ? (
            ...
            <div className="space-y-8">
              ...
              <Section title={t("moderation")}>
                <div className="grid gap-2 rounded-xl border bg-muted/30 p-3 text-sm sm:grid-cols-2">
                  <p className="flex items-center gap-2">
                    ...
                    {t("updated")}: {dateFmt(data.service.updated_at)}
                  </p>
                  <p className="flex items-center gap-2">
                    <Clock className="size-4 text-muted-foreground" />
                    {t("status")}: {statusLabel(t, data.service.status)}
                  </p>
                  {reviewedDate && (
                    <p className="flex items-center gap-2">
                      <Clock className="size-4 text-muted-foreground" />
              ...
              {/* Service info */}
              <Section title={t("serviceInfo")}>
                <div className="space-y-2">
                  <InfoRow value={data.service.description} label={t("description")} />
                  <InfoRow
                    value={data.service.category ? localizedName(data.service.category, locale) : null}
                    label={t("category")}
                  />
                  {data.service.price != null && (
                    <p className="flex items-center gap-2 text-sm">
                      <span className="mr-1 text-muted-foreground">{t("price")}:</span>
                      {priceFmt(data.service.price)}
                    </p>
                  )}
                  {data.service.duration_minutes != null && (
                    <p className="flex items-center gap-2 text-sm">
                      <span className="mr-1 text-muted-foreground">{t("duration")}:</span>
                      {data.service.duration_minutes} {t("minutes")}
                    </p>
                  )}
                  {data.service.slug && <InfoRow value={data.service.slug} label={t("slug")} />}
                  {Array.isArray(data.service.tags) && data.service.tags.length > 0 && (
                    <InfoRow value={data.service.tags.join(", ")} label={t("tags")} />
                  )}
                </div>
              </Section>
